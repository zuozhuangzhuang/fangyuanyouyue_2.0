...
//			Map m = parseXmlToList2(notityXml);
    ...
    @ApiOperation(value = "删除官方鉴定", notes = "删除官方鉴定",response = BaseResp.class)
    @ApiImplicitParams({
            @ApiImplicitParam(name = "token", value = "用户token",required = true, dataType = "String", paramType = "query"),
    ...
    @ResponseBody
    public BaseResp deleteAppraisal(GoodsParam param) throws IOException {
        try {
            log.info("----》删除官方鉴定《----");
            log.info("参数：" + param.toString());
            //验证用户
            ...
            //删除全民鉴定
            appraisalService.deleteAppraisal(userId,param.getIds());
            return toSuccess();
        } catch (ServiceException e) {
            e.printStackTrace();
            return toError(e.getMessage());
